...
public class AudioRecordManager implements Handler.Callback {
    ...
    public void setAudioSavePath(String path) {
        if (TextUtils.isEmpty(path)) {
            this.SAVE_PATH = mContext.getFilesDir().getAbsolutePath();
        } else {
            this.SAVE_PATH = path;
        }
    }
    ...
    private void startRec() {
        ...
        try {
            ...
            //设置文件的输出格式
//            audioRecorder.setOutputFormat(MediaRecorder.OutputFormat.AAC_ADTS);//多加

            //设置音质频率
            this.mMediaRecorder.setAudioEncodingBitRate(96000);

            this.mAudioPath = Uri.fromFile(new File(SAVE_PATH, System.currentTimeMillis() + ".wav"));
